...
class OptimizationSettings:
    ...
    max_avg_walking_dist_m: float = 250.0
...
def filter_grid_feasible_locations(
    ...
    """Keep feasible candidates; keep unknown-postcode candidates as usable."""
...
def total_raw_service_capacity(
    number_chargers: int, settings: OptimizationSettings
) -> float:
    ...
    return (
        number_chargers
        * settings.connectors_per_new_charger
        * settings.service_capacity_per_connector
    )
...
def evaluate_locations(
    ...
    raw_capacity = total_raw_service_capacity(len(selected_locations), settings)
...
def is_feasible(kpis: OptimizationKpis, settings: OptimizationSettings) -> bool:
    return (
        ...
        and kpis.avg_walking_dist_m <= settings.max_avg_walking_dist_m
...
def optimize_fixed_number(
    ...
    """Level 2: choose the best new locations for a fixed number of chargers."""
...
def find_minimum_feasible_chargers(
    ...
    """Level 1: find the minimum new charger count satisfying the KPI thresholds."""
    feasible_candidates = filter_grid_feasible_locations(
        candidate_locations, grid_feasibility
    )
...
def select_optimized_locations(
    ...
    """Bridge function used by the simulation to request optimized locations."""
    service_capacity = config.get("service_capacity_per_connector")
    ...
    settings = OptimizationSettings(
        ...
        max_avg_walking_dist_m=config.get("max_avg_walking_dist_m", 250.0),
        ...
        service_capacity_per_connector=service_capacity,
